...
global equ;
global fixed;
global flag; global A; global counter;
fixed=1;
[value,max,es]=GetInputs('FixedPoint',flag,A,counter);
if(flag==1)
    flag=0;
    %A=get(handles.pushbutton43,'A');
        equ=str2func(['@(x)',char(A(1))]);
    if(strcmp(equ, '')==0)
    if(size(value)>0)
...
global flag; global A; global counter;
setting();
[value,max,es]=GetInputs('NewtonRaphson',flag,A,counter);
if(flag==1)
    flag=0;
    if(size(value)>0)
        %A=get(handles.pushbutton43,'A');
...
global flag; global A; global counter;
setting();
[value,max,es]=GetInputs('Bisection',flag,A,counter);
if(flag==1)
    flag=0;
    if(size(value)>0)
         %A=get(handles.pushbutton43,'A');
...
global flag; global A; global counter;
setting();
[value,max,es]=GetInputs('FalsePosition',flag,A,counter);
disp(max);disp(es);
if(flag==1)
    flag=0;
    if(size(value)>0)
...
global flag; global A; global counter;
setting();
[value,max,es]=GetInputs('Secant',flag,A,counter);
if(flag==1)
    flag=0;
    if(size(value)>0)
        %A=get(handles.pushbutton43,'A');
